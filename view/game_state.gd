...
# 流程参数继续用静态变量跨场景；在线会话/配置/经济统一由 autoload `Online` 持有。
extends RefCounted
...
static func config():                   # 在线运行用服务器 ConfigPush；无 SceneTree 的纯逻辑测试才回退仓库 JSON
	var online = _online_runtime()
	if online != null:
...
static func economy():                  # EconomyStateCache（服务器经济/养成状态非权威缓存）
	var online = _online_runtime()
	if online != null:
		return online.economy()
	if _economy == null:
		_economy = load("res://net/economy_state_cache.gd").new(_net_api_url())
	return _economy

static var _kingdom = null
static func kingdom():                  # KingdomStateCache（K2：王国领地非权威缓存）
	var online = _online_runtime()
	if online != null:
		return online.kingdom()
	if _kingdom == null:
		_kingdom = load("res://net/kingdom_state_cache.gd").new(_net_api_url())
	return _kingdom


static func is_online_ready() -> bool:
	var online = _online_runtime()
	return online != null and online.is_online_ready()


static func _online_runtime():
	var main_loop := Engine.get_main_loop()
	if main_loop is SceneTree:
		return (main_loop as SceneTree).root.get_node_or_null("Online")
	return null

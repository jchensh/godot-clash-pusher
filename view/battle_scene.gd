...
var match_obj
var selected_card := -1
var _level_id := "level_01"   # 本局关卡（来自选关界面），仅显示/接线用
var _ai_diff := "normal"      # 本局 AI 难度（随关卡而定），仅显示/接线用
...
func _ready() -> void:
	...
	add_child(fx_layer)
	_build_hud()
	_log("MATCH START  %s | 3 lane | AI=%s" % [_level_id, _ai_diff])
...
func _build_topbar() -> void:
	...
	var diff_lbl := _label("%s | AI %s" % [_level_id.to_upper(), _ai_diff.to_upper()], Vector2(0, 60), 18, Color(0.72, 0.78, 0.72))

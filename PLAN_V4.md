...
| **V4-S12** | 好友/聊天/俱乐部（可选） |
| **V4-S13** | **断线托管 AI（disconnect bot takeover）**：一方掉线后，在重连窗口内不再「干等 / 超时认输」，给其阵营接管一个**弱 AI**（出兵节奏故意放慢、决策简单，复用单机 `ai/ai_controller.gd` 低档思路，见「难度=AI 节奏」）顶替继续打；该方重连后交还控制权。⚠️ lockstep 关键约束：bot 的 deploy 必须由**服务端权威注入**且确定性（两端同 tick 应用、纳入 state hash 对帐），**不能两端各自本地跑 bot**（否则 desync）。建立在现有 V4-S3f 重连（room TTL 60s）+ 超时认输之上。**当前不做**，PVP 体验打磨阶段再启动（用户 2026-06-29 提出）。 |
...
- **断线托管 AI（V4-S13）设计点**：bot 接管/交还的触发时机（掉线即接管？还是宽限 Ns？）；bot deploy 如何由服务端确定性注入到 TickBundle 并纳入 hash；bot 用哪档难度（要明显弱）；重连后如何无缝交还（V4-S13 前定）

...
- **V3 进行中**（战斗核心 2D 重构 + 买断制单机，权威规划 [PLAN_V3.md](PLAN_V3.md)，方向见决策 36/37）：
  ...
  - V3-4a ✅ Roguelite 骨架（RunState + 节点地图：线性连战链 + 连战流转：二元永久死亡；3 act×3 战，决策 38）
  - 单测 **144/144**；**V3-1h / V3-2 / V3-3 的画面·手感留真人实机验收**（首次可玩 2D 重构）。
- **Now**：下一步 **V3-4b draft 三选一**（复用 `deck_builder` 逻辑、改写本 run 卡组）。

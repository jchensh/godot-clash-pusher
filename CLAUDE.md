...
- **V1 收官**。现进入 **V2**（顺序 A→D→B→C，权威规划见 PLAN_V2.md）：
  ...
  - V2-7 🔄（C 模块 内容/数值）按一步一确认拆 3 小步（决策 34）：7a 扩卡池 / 7b 多关卡+选关（关卡=独立遭遇战含难度，取代难度界面）/ 7c 组卡界面（自由选 8 张）。
    ...
    - V2-7c ✅ 组卡界面（`deck_builder`，卡池动态读取、自由选 8 张唯一卡、预填关卡默认；经 `GameState.player_deck` 覆盖关卡默认，`Match.setup` 加可选覆盖参数、仅作用玩家卡组）。单测 **121/121** + 端到端烟测（自定义卡组进对局玩家手牌生效）。**至此 V2-7 收尾完成**，流程：菜单→选关→组卡→对局→结算。**Now：V2-8 数值平衡 pass**，或择期补 5c 音频。

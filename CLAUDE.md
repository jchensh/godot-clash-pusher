...
- **V1 收官**。现进入 **V2**（顺序 A→D→B→C，权威规划见 PLAN_V2.md）：
  ...
  - V2-7 🔄（C 模块 内容/数值）按一步一确认拆 3 小步（决策 34）：7a 扩卡池 / 7b 多关卡+选关（关卡=独立遭遇战含难度，取代难度界面）/ 7c 组卡界面（自由选 8 张）。
    ...
    - V2-7b ✅ 多关卡（1→**4 关**，各自带难度/节奏/AI 卡组）+ 选关界面（`level_select`，按难度档卡片，**取代难度选择界面**；难度内嵌进关卡、经 `GameState.level_id` 传递）。逻辑零改动，单测 **120/120** + headless 烟测（level_03 AI 部署新卡零报错）。流程：菜单→选关→对局。**Now：V2-7c 组卡界面**（自由选 8 张），或择期补 5c 音频。

...
- **V1 收官**。现进入 **V2**（顺序 A→D→B→C，权威规划见 PLAN_V2.md）：
  ...
  - V2-6 ✅（B 模块）规则 AI 升级：攻防结合（受威胁 lane 优先空投拦截兵）+ 按 lane 选进攻方向（集火最弱敌塔）+ `ai_difficulty` 难度 easy/normal/hard 分级（阈值/间隔/是否防守/选路 4 维度），逻辑层 + 单测 116/116（决策 33）。另加**难度选择界面**（主菜单→难度→对局，仅显示层）。**Now：C 模块 V2-7**（内容/数值），或择期补 5c 音频。

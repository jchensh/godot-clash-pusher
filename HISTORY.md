...
### V5 · 全局横屏 L3+L4 前半：战斗真横屏 + 主界面/闯关图原生横屏布局（✅ 真人验收通过，2026-07-26）

- **L3 战斗真横屏**（验收两轮：首轮 HUD 浮层压地图被打回 → 改独立带）：全局横屏下战斗一律横版投影（我左敌右；唯教程锁竖版走 fit 壳）；场区=顶栏/手牌带之外 zone 内取整方格居中（1280×720 → 27px/格 864×486，选卡区独立成带**恒不压地图**）；HUD 卡面/圣水条居中 720 块；Router 单机战斗不包壳。**自检抓到 0715 潜伏崩溃**：无河桥场地（V1 遗留关）BG 特征对齐 bridges[0] 越界——两场景加回退瓦片保护。圣水条+下一张 chip 双场景重复代码收口 `HudWidgets.draw_elixir_row`。
- **L4 原生横屏 UI（mockup 先行已评审：docs/design/ui_mockups/landscape_ui.html）**：设计原则=竖屏「上下堆叠」→横屏「左导航·中主体·右辅助」三栏。已实装两页（Router LANDSCAPE_NATIVE 白名单逐页豁免包壳，竖屏布局原样保留）：
  - **主界面**：顶带 72（名片/钱包/公告/设置）+ 左导航栏 120（六页签竖排，主页金色高亮）+ 中主体（章节主视觉 760×352 + 进度条 + 布阵/国王征途/对战三大入口）+ 右辅助栏 330（挂机金库卡/探险卡），一屏无滚动。
  - **闯关图**：顶带（返回/标题/状态/钱包 560 右贴边）+ 左章节导航列 300（当前章金色高亮、锁章灰、默认跟随下一关）+ 右关卡 4 列网格卡 219×190（点章节切网格）；战后结算/开箱/重试门零改动。
  - 踩坑：编辑器嵌入调试窗口不吃游戏改窗口尺寸 → 用户已取消 Embed Game on Next Play，独立窗口自动 1280×720↔720×1280；MCP 注入键盘无 unicode 打不了字（真人无碍）。
- **测试**：客户端 **435/435**；gdlint 绿。真人验收：战斗横屏/主界面/闯关图三屏通过（闯关图右上钱包溢出当轮修复）。待续：deck_builder/card_collection/settings/kingdom 横屏 + net_battle 横版（H6）。

### V5 · 全局横屏 L4 续：组卡 + 图鉴原生横屏（✅ 真人验收通过，2026-07-26）

- **deck_builder 横屏**：顶带（返回/「上阵」/场景副题/已选计数）+ **左卡组面板 360**（8 格 2×4 + 战力 + 出征/保存 + 满8提示，目标区固定左手边不随滚动）+ **右卡池 5 列**网格（880 宽滚动窗；竖屏 4 列）。选卡逻辑/_refresh 零改动。
- **card_collection 横屏**：顶带（返回/「养成」/解锁状态/钱包 560 右贴边）+ 排序条一行（y84，控件逻辑复用）+ 卡片 **4 列**网格（竖屏 2 列，一屏 8 张）。
- Router LANDSCAPE_NATIVE += 两页；竖屏布局原样。前轮遗留：闯关图右上钱包溢出（wallet_bar 按传入 560 自绘）→ 右贴边 x=1280-560-14 修复，成为横屏顶带钱包标准姿势。
- **测试**：客户端 435/435；gdlint 绿。真人验收通过（组卡选卡/出征、stage 副题、图鉴排序/4列、钱包完整、竖屏回归）。

### V5 · 全局横屏 L4 收尾：设置/王国/卡详情原生横屏 + 图鉴补费用（✅ 真人验收通过，2026-07-26）

- **settings 横屏两栏**：左=设置组（战斗版式/屏幕方向/语言 + 登出|返回并排）、右=GM 区三列 12 键整体右移（y108 起 4 行），一屏免滚。
- **kingdom 真横屏主城**：新增 SLOTS_LAND/PLAZA_LAND/TREE_DECO_LAND 横屏布局表（王城顶中、生产两翼、城防近前门、广场居中），槽位引用全部收敛 `_slots_map/_plaza/_trees` 运行时映射——路网/巡游小人/Y-sort/点击检测自动跟随；地形按 1280×720 铺装；HUD 顶带 92px 一行排开 + 收取钮右侧 + 返回左下。注：待正式场景资源到位会大换/重构（用户口径），当前为可用版。
- **card_detail 居中列**：内容块（左立绘右属性，y84~700）整列平移 280px 居中呈现零改动 + 顶带返回/钱包 560 右贴边。
- **图鉴补费用**（验收反馈）：卡面左上角紫底圣水费药丸（横竖两版同享）。
- Router LANDSCAPE_NATIVE 至 7 页；剩余过渡态=登录/创号/level_select/战役演出（居中式立柱观感可用）；net_battle 横版=H6 两机验收另排。
- **踩坑**：会话中发现工作树处于 detached HEAD（L1~L4 三个 commit 游离）——master 是祖先，fast-forward 合回并推送恢复。
- **测试**：客户端 435/435；gdlint 绿。真人验收：设置/王国/卡详情/图鉴费用通过。

### V5 · H6 联机 PVP 横版：横版投影 × side2 复合（🚧 代码+自测完成，两机真人验收挂账，2026-07-26，KAN-115）

- **复合方案**：沿用竖版 side2 已验收的逻辑 180° 翻转（_t2s/_s2t 翻转段零改动），再套横版投影——等价施工图「横投影+左右镜像」+上下换道（换道语义与竖屏 side2 一致；场地双桥/河两轴对称，BG 不参与翻转仍对齐）。KAN-49 教训的复合易错面被压到最小。
- **改动**（net_battle_scene 单文件 + Router 一行）：_field_rect/_t2s/_s2t/_tile_px 横版分支（1280×720 → 27px/格 864×486 zone 内居中）；新增 _fp_screen（footprint 方向语义）；_screen_up_tiles 复合方向（side2 横版=逻辑 +x）；部署高亮改左半场（本方恒左）；BG 绕场心 90° 旋转分支（同单机 H2 公式）；HUD 手牌/圣水/名片居中 720 块；Router 联机战斗解包。
- **顺手修潜伏 bug**：tile 瓦片回退绘制 side2 竖版偏移一格（_t2s 翻转后非左上角）→ 改对角投影包围盒取法（BG 常开从未暴露）。
- **自测**：新增 `test_net_hbattle_transform` 6 条——**本方底线恒屏幕左缘（side1/side2 双视角锁定）**、部署分界线两视角同一屏幕 x、坐标往返互逆（双视角×4 采样）、方向 API、竖版公式零回归；全量 **441/441**、gdlint 绿。
- **挂账**：两机真人验收（朝向不反/落点合法/本方数据正确/全程 hash 零 mismatch），用例并 KAN-76 台账。Jira：KAN-115 → 审查中。

### V5 · 王国正式城建场景接入：0726 素材批次 BG 整图 + 16 建筑（✅ 真人验收通过，2026-07-26，KAN-112）

素材源 = `testAssets/城建0726`（BG 城内俯视全景 1440×1830 + 15 建筑 + 「未建造」空地图 + 角色大小示意=布局参考）。

- **映射拍板（用户）**：keep=主公府 / farm=农田 / workshop=伐木场 / granary=仓库 / mint=市场 / wall=城门；watchtower 沿用 0721 老图；四兵营（步/枪/弩/骑）+ 采石场/石厂/炼铁厂/牧场/商店 = **9 栋 placeholder 新建筑**。
- **配置（双端共享零服务器代码）**：kingdom.json 7→16 建筑，placeholder kind 标记、1 级、粮草/木石 100~300、60~120s（成本禁 gold 铁门照守，服务器 validate 全通用）；api/gateway 重启日志确认「16 buildings」解析、verifier 按铁律重启。老玩家零迁移（新建筑默认 0 级=空地可建造）。
- **kingdom.gd 场景层重构**：tile 地形/代码路网/代码树全下线 → **BG 整图 + 单一视野变换 _k2s**；16 槽位统一 BG 像素坐标系（按示意图落位 + 脚本实测校准）；**横竖屏同缩放 0.889**（验收反馈：竖屏拉近对齐横屏观感），竖屏双向拖 / 横屏上下拖（拖动阈值 12px 区分点选，_pan 钳制图内）；开局视野对准主公府；空地=「未建造」贴图；**placeholder 建成后点击=敬请期待 toast**（config kind 驱动）；小人改建筑间漫游。
- **验收微调三则**：①城门对齐三轮返工后改「先量后解」——pillow 实测路中心 708.5/墙基线 1530 + 贴图内旗杆锚点 x=102（≠画布中心，旗面偏飘导致目测总偏），反解 (702,1558) 一次到位（教训沉淀 agent 记忆 asset-alignment-measure-first）；②箭塔挪城墙四角（主槽=左下角挂名牌/倒计时，三角视觉复刻建成现身，**下方两角均为交互入口**）；③竖屏缩放统一。
- **测试**：客户端 **442/442**（+1 城建扩容账本：16 建筑/placeholder kind/仅 1 级）；gdlint 绿；Go kingdom 包过。真人验收通过（场景/落位/placeholder 建造链路/老功能回归/横竖两版）。

### V5 · 决策 49：卡通改版立项——题材三国→卡通 + 竖屏横板战斗 + 回归竖屏（📋 施工图+建单，2026-08-30）

- **拍板**（推翻三国题材线与全局横屏方向）：①全局回归竖屏、横屏线（L1~L4/H5/H6 横屏壳）封存（代码留 git 入口下线）；②viewport 750×1334；③首发缩池 21 卡通角色+法术、其余单位卡锁定（配置/技能全保留等美术）；④城建缺素材建筑顶替（魔法书院→步兵营位、科研所→石厂位、药剂研究→铸币坊位）；⑤帧数自动检测切帧；⑥敌方塔先程序调色（美术欠账记录）；⑦三国彻底下架（git 留档，不做运行时切换）。
- **素材源**：`testAssets/新风格美术资源0830`（101 PNG：战斗 BG 750×1625+格子 30×26@25px+王塔/箭塔 2 倍交付；21 角色×{立绘/攻击/行走} 正面 3/4 视角横排帧；城建拼接件 33 张示意 1440×1830 与现王国 BG 同尺寸）+ 飞书《城建资源说明》《尺寸格子说明》两文档（bot 身份可读）。
- **技术路线**（详见 [PLAN_V5_CARTOON.md](PLAN_V5_CARTOON.md)）：逻辑层保持竖版语义仅改 arena.json 盘面 18×32→26×30（河 2 行/桥 3 格/塔位重排，lockstep 输入变更=历史 PvE 重放作废、verifier 重启）；渲染默认走 H1/H2 收敛好的横版投影（我左敌右）+H6 side2 复合，横屏线的变换层资产全复用；sprite_db 对新素材简化为单行正面帧+flip_h。
- **施工分期**：P1 战斗竖屏横板（8 步）→ P2 题材文本换皮 → P3 王国拼接式重摆（K7 设计稿届时按新题材改写）→ P4 美术欠账收尾。本步=P1-1 施工图+HISTORY+Jira 四单。

### V5 · 卡通改版 P1-2：arena.json 新盘面 26×30（✅ 442 绿，2026-08-30，KAN-121）

- **配置**：grid 18×32→26×30；河 y[15,17)→[14,16)（2 行居中）；桥 2 格宽 x[3,5)/[13,15)→**3 格宽 x[0,3)/[23,26)**（贴场地上下边，对齐新 BG 桥位）；部署线 17/15→16/14；塔位重排：王 (9,29)/(9,3)→(13,27)/(13,3)，公主塔 (4.5,24)&(13.5,24)→**桥头后方 (1.5,22)&(24.5,22)**（敌方对称 y=8）。tower_combat 数值零改动。arena.json 不进 Excel 镜像，无需 build_config。
...
### V5 · 卡通改版 P1 首轮验收修复：BG 对齐/体型标准/肖像补丁（✅ 448 绿待复验，2026-08-30，KAN-121）

- **验收结论**：A 竖屏壳/C 缩池全过；B 战斗流程通，报三问题，全部定位修复：
- **①"单位河上走+桥错位"= 战斗区对齐假设错**：P1-4 曾设战斗区在 BG 垂直居中（y=487.5 起）——pillow 实测（格子示意罩层边界 + 双桥木色反解）真值 **y=425 起**，且 BG 双桥**非贴边**：上桥 x∈[3,6)、下桥 x∈[20,23)（关于 x=13 镜像对称 ✓）。旧配置桥贴边 [0,3)/[23,26) → 单位走逻辑桥时画面正是河/草地。修：CARTOON_BG_FIELD 425 + arena.json 桥重定 + 公主塔挪桥头 x=4.5/21.5；测试点 (9,17) 系因塔近中路进塔火圈 → 全部挪中线 (13,17)。教训沉淀：**"居中"是假设不是测量**——对齐类参数必须 pillow 实测（asset-alignment-measure-first 再次应验）。
- **②单位体型失控（马蜂王 9 格巨蜂）= "素材÷2 直画"标准废弃**：美术各角色源图分辨率不统一（蟑螂 34px vs 马蜂王 217px），直画即失控。新标准 = **体型表驱动等比适配**：显示身高 = UNIT_VIS 半径×2×格×CARTOON_H_MULT(1.75)，像素比例 k=身高/walk 帧高（attack 帧同 k 放大画布——本体不跳变、武器自然外扩），底边贴脚线、影子按 body 基准；素材高清只作超采样。派生形态 scale 归一（体型差全由 UNIT_VIS r 表达）。
- **③组卡/图鉴旧图 = P1-6 补丁静默失败**：python replace 因缩进不匹配没改到 `card_portrait_tex`（战斗手牌走 frame() 已新、肖像走旧 DB）——headless probe 实锤后精确修复，肖像全返卡通立绘；测试改"以立绘文件为真相源"回归锁。教训：**批量文本替换必须带 assert 命中**（本轮新补丁已全部加 assert）。
- **飞斧汪远程 = 原设定非 bug**（axe_thrower 射程 4.5 扔斧，三国巴郡飞斧手同数值；改近战属数值变更需另拍板）。
- 全量 **448/448**；gdlint 绿；verifier 重启（验收期旧局已在改前标 skip，本轮 0 增量）。台账补 0830 首轮结论，待复验 B-1/2/3/4/5。

### V5 · 卡通改版 P1 收官：真人验收通过（✅ Done，2026-08-30，KAN-121）
...
### V5 · 卡通改版 P2：题材文本换皮 三国→卡通（✅ 450 绿·Excel 同步完成，2026-08-30，KAN-122）

- **游戏名（用户拍板）**：「乱世推塔」→「**萌萌对垒**」（en: Cuddly Clash；副题「萌萌王国·推塔对战」）；project.godot 窗口名同步。
- **48 卡名全换（i18n zh+en 双语 + cards.json name 同步）**：21 张有素材卡沿用美术命名（小小/兔射手/鼹鼠火枪手/飞斧汪/猪猪配香菇/火又鸟/幺蛾子/马蜂王/鸦王/岩石·锤锤·棒槌史莱姆/瓜牛炮/蝴蝶仙子/蟑螂小队/蟑螂恶霸帮/双斧火钳/猫炮手/火·雷精灵/小公主）；17 张锁定卡起中性卡通名（大块头/小乌鸦/龙宝宝/冰精灵…素材到位可再调）；10 法术直白功能名（大火球/箭雨/电击术/滚木/冰冻术/蟑螂空投…）。
- **10 章名重起**（卡通冒险线贴 0830 BG 世界观：绿绿草原→蘑菇森林→潺潺河谷→花花平原→迷雾沼泽→石头山道→松林雪线→雪山之巅→熔岩边境→王城决战；i18n zh/en + stages_spec 同步 + build_stages 重出，test_chapter_names 一致性锁过）。
- **units.json 39 单位名**全换（派生形态派生命名：火又鸟·重生/幼蛾子）；**教程文案** 2 条去三国味（"乱世烽烟起"→"欢迎来到萌萌王国"）。遭遇模板=纯卡组零文案不涉及；faction 字段零显示引用不动（未来阵营语义保留）；卡详情/图鉴立绘 P1 已走卡通层零改动。三国文案 git 留档下架 ✓。
- 客户端 **450/450**（含章节名 spec↔i18n 一致性）；容器已重启（bundle 新版本下发）。GameConfig.xlsx 同步完成（用户关闭工作簿后 --from-json + --check 过）；卡名对照表经用户过目认可。

### V5 · 卡通改版 P3：王国拼接式城建重摆（🚧 450 绿待真人验收，2026-08-30，KAN-123）

- **拼接式场景**（BG 整图下线，对齐美术《城建资源说明》「均由独立物件拼接」）：地表 256×512 色块 tile 错位平铺（奇数列上移半块=「错位一格再复制」）+ 围墙程序圈（横段/竖段平铺+绿旗角柱四角+底墙城门开口）+ 路网 9 段布局表（竖段平铺/横段旋转 90° 复用）+ 装饰 16 散点（树花竹桶+南瓜居民屋大装饰）。全经 _k2s 视野变换，拖动/缩放/Y-sort/小人漫游架构零改动。
- **16 建筑卡通贴图落位**（assets/kingdom_cartoon/ 32 件入库，示意图坐标手标）：四顶替落定——workshop=紫蘑菇「魔法工坊」、stoneworks=天文台「科研所」、mint=瓶子屋「药剂工坊」、camp_infantry=梯子屋「魔法书院」；wall 槽=城门贴图（沿 0726 先例）、watchtower=战斗箭塔复用（风格统一）。kingdom.json display_zh 16 处改名（key 零改动=服务端零代码，容器已重启下发）。
- **K7 设计稿（§11）按卡通术语改写完成并入库**（2026-07-27 三国稿→机制不变换皮：troop_class `infantry`→`mage` 法系随魔法书院改义、粮草→食物、互市→资源交换、王城/箭楼→城堡/箭塔；31 可玩卡先定表口径补注）。
- 客户端 **450/450**；gdlint 绿；headless import 零错。验收台账追加 E 组 4 例（含已知过渡：未建造空地旧图/路网第一版粗摆）。

### V5 · 卡通改版 P3 验收修复：城门墙缝/路网重构/横路漂移（✅ 450 绿待复验，2026-08-30，KAN-123）

- **①城门两侧墙缝**：底墙开口原"整段跳过"（167px 粒度）致缺口远大于门体 → 改**精确裁剪**（重叠段裁成左右两截、src x 随裁剪偏移保纹理连续；开口 220 < 门贴图 253 骑缝盖缝）。
- **②路网与建筑错位**：手标 ROAD_SEGS 弃用 → **运行时按 SLOTS 生成**（竖主干 城堡门→城门 + 中/下横干 + 9 栋门口垂直支路；下排四营/药剂工坊门口贴下横干免支路）——建筑挪位路自动跟随，永不再错位。
- **③左下角路随镜头漂**：横路旋转绘制的局部系数学错误（draw_set_transform 已含段中心平移、又把中心加进局部坐标=双重平移，随 _pan 变化漂移）→ 局部系只乘 _scale 修正。
- **顺手修**：视野钳制旧硬编码 720×1280（P1-3 改 750×1334 后拖到边缘露 30px 夜色底）→ get_viewport_rect() 动态跟随。
- 450/450；gdlint 绿。待复验 E-1（墙缝/路网/漂移三点）。

### V5 · 卡通改版 P3 收官：真人复验通过（✅ Done，2026-08-30，KAN-123）

- E 组 4 例全过（三修复生效：城门墙缝精确裁剪/路网 SLOTS 生成/横路漂移局部系修正）。KAN-123 → Done。**卡通改版 P1/P2/P3 全线收官**，剩 P4 美术欠账（KAN-124）随素材批次逐项接入。

### V5 · 卡通改版 UI 换肤：天空牧场淡色系 + 弃像素风（🚧 450 绿待真人验收，2026-08-30，KAN-125）

- **方案流程**：三套淡色方向对比稿（设计画布 artifact a5e00b52：A 奶油甜点/B 天空牧场/C 纸绘童话，各含主菜单+组件板 mockup）→ **用户拍板 B「天空牧场」**（淡天蓝底/白卡面板/暖黄 CTA/天空蓝标题）。实现路线=纯程序化（现有 UI 管线本就是 gen 脚本生成，零手绘素材、无需图像生成 connector）。
- **gen_ui_assets.py 重写**：夜色方形像素 9-slice → 天空牧场**圆角卡通**（4× 超采样抗锯齿圆角矩形+外描边+内上亮/下暗立体带；三套按钮三态+白卡/凹槽面板，色值与设计稿方案 B 逐一相同）；主菜单背景 → 天空→草地渐变+程序云。
- **pixel_ui.gd**：色板 6 常量换血（主字深青蓝/标题天空蓝/描边深蓝；常量名沿用引用面不动）+新增 COL_PANEL_BG/COL_INSET_BG；sbpixel 圆角 10（弃像素方块）；9-slice 边距适配圆角半径。
- **弃像素风（用户明确）**：字体 fusion-pixel 像素字 → **站酷快乐体**（ZCOOL KuaiLe，OFL 开源可商用，与设计稿同款）；project.godot 全局主题+5 处 preload 共 6 处替换。
- **散装夜色清算**：view/ 15 文件深色 hex 按语义映射（最深底→淡蓝灰/深面板→白卡/深棕描边→淡蓝/旧金强调→天空蓝）；战斗 HUD 常量双场景淡色化（顶栏/手牌板/卡面/选中金）；手牌卡名白字→深青（淡卡面可读）。
- 客户端 **450/450**；gdlint 绿；headless import 零错。**真人验收待执行**（全页面走查+战斗 HUD 可读性；设计稿↔实机一致性）。

### V5 · 卡通改版 UI 换肤收官：天空牧场 + 对比度修复（✅ Done，2026-08-30，KAN-125）

- **首轮验收反馈=对比度不足三处，全部根因修复**：①禁用按钮字隐身——style_button 从未定义 font_disabled_color（引擎默认半透明白，深底时代掩盖）→ 显式深青 45% 全局生效；②标题重影+压副题——4-label 偏移描边工艺是像素字遗产（3px 错位在圆体下成重影、快乐体行高更高压到副题）→ **11 处 _title 全换引擎原生 outline**（天空蓝字+白粗描边，任何底色可读）+图鉴副题下移；③战斗 HUD 字浅——阵营色/倒计时白字是深底配色 → COL_PLAYER/OPPONENT(+net SELF/FOE) 加深、倒计时深青（30s 红脉动保留）、手牌卡名加深加号。二轮验收通过 → KAN-125 Done。
- **踩坑沉淀**：批量替换静默失败教训进 agent 记忆（batch-edit-must-assert：每条 assert 命中+grep 归零+headless probe 实证）；"居中是假设不是测量"追加进 asset-alignment-measure-first；CLAUDE/AGENTS 配置工作流补「Excel 开着锁文件」提示（check_docs 18 文件过）。
- **卡通改版整线收官**：P1 战斗竖屏横板 / P2 文本换皮 / P3 王国重摆 / UI 换肤（KAN-121/122/123/125）全 Done；剩 KAN-124 美术欠账随素材批次 + D-1 两机联机后补。客户端 450/450。

### V5 · 卡通改版 P4 欠账补位：塔阵营配色套 / 王塔废墟 / 空地卡通版（✅ 真人验收通过，2026-08-30，KAN-124 部分）

- **选题原则**：KAN-124 大头（17 锁卡素材/死亡帧/法术 FX/多场地 BG/音频/闯关图）**依赖新素材到货**推不动；本轮只做**不依赖素材、程序可补**的三项，且照 `gen_ui_assets.py` 先例走**离线生成 + 产物入 git**，而不是运行时糊调色。美术出正式素材后覆盖同名文件即可、代码零改。
- **①塔阵营配色套**（`tools/gen_cartoon_towers.py`）：素材实测推翻原方案——卡通塔主色**本就含高饱和红**（箭塔顶 `208,32,32`、王塔暗红 `112,0,0`），原先"敌方整体乘红 0.40"导致**敌我都偏红**且石墙木头一起脏。改为**离线派生蓝顶套**（红带 h∈[330,20]∧s≥0.35 → 旋到 h=212，石头/木头/米色天然落带外不受影响）→ 经典红蓝对立；运行时 tint 双方同归 0.12 轻染。改色量：王塔 7.9% / 箭塔 20.1%。
- **阵营指派用户拍板（同日二轮）**：首版按素材原色定为「我方红/敌方蓝」，但与 HUD 阵营色（`COL_PLAYER`/`COL_SELF`=蓝=我、`COL_OPPONENT`/`COL_FOE`=红=敌）相反 → **拍板对调塔色**：**我方=蓝顶（派生套）/ 敌方=红顶（美术原生）**，全局阵营语言统一为蓝=我红=敌。派生件随之改名 `_enemy`→`_blue`（它现在是我方的皮，`_enemy` 会误导后来人）。
- **②王塔废墟**（同脚本）：原先与箭塔废墟共用一张，**王塔倒下=决胜瞬间却看不出区别**（几何上并无拉伸，是辨识问题）。沿用美术在箭塔废墟里定下的「废墟语言」（俯视空壳+锯齿断口+石板地+散落木梁）放大到王塔尺寸，再把王塔金王冠抠下来蒙尘（去饱和 15%+压暗 18%+补深褐描边）倾倒摆进内院 → 一眼读出「王塔倒了」。废墟无阵营色 → 敌我共用一张，常量 `TEX_TOWER_KING_{MINE,ENEMY}_BROKEN` 收敛为单个 `TEX_TOWER_KING_BROKEN`。
- **③未建造空地卡通版**（`tools/gen_kingdom_plot.py`）：P3 王国换卡通拼接后空地仍是 0726 灰石写实工地，突兀。程序化画「围起来的待建工地」（黄土地块+四角木桩+麻绳+石料木料堆+碎石），**色值逐个取自 0830 卡通城建素材本身**（黄土 `#d8b460` 同 road、栅栏木 `#b46c24` 同 farm、石灰 `#848484` 同 quarry）保证同族；画法照 gen_ui_assets（4× 超采样+圆角+粗描边）。
- **踩坑**：①抠王冠时混进孤立金噪点，描边后在废墟里变成小黑点 → 加**最大连通块过滤**；②`crown` 按左上角定位导致王冠掉到门洞边缘 → 改**按中心**定位；③Windows 控制台 cp1252 编码中文 print 直接崩 → 脚本统一 `sys.stdout.reconfigure(encoding="utf-8")`。两个生成脚本均按记忆纪律**带命中断言**（改色像素数下限 / 王冠像素数下限 / 产出不透明比例），阈值不达标直接拒绝产出。
- **验收结果**：G 组 G-1~G-6 功能逻辑真人验收通过（G-7 联机并入 D-1 后补）；阵营色按拍板对调后复跑全绿。
- 客户端 **450/450**；gdlint 绿；check_docs 18 文件过；headless import 零错（3 张新贴图 `.import` 已生成）。台账追加 G 组 7 例。
- **剩余 KAN-124 欠账**（仍待素材）：17 锁卡素材、死亡帧、法术 FX/图标、多场地 BG、卡通音频、闯关图/主菜单美术。
---
...
### V5-E2 / Release 76243c8 — 公网测试包部署与服务端数据库 schema v9 升级 (2026-07-21)
- **部署内容**：
  - 合并 `master` 0721 素材批次与王国领地系统至 `release` 分支（提交哈希 `76243c8`）。
  - 服务端升级：王国领地模块 `internal/kingdom` + `/v5/kingdom/*` API、K4 PVE 塔加成、K5 PVP 城防下发 (`battle.proto`) 及 username 登录重试门修复。
  - 客户端更新：0721 正式美术素材大批次（7 建筑 + 装饰树、绿地图与地块重标、废墟贴图、5 角色/兵种帧动画及投射物/爆花）。
- **服务端部署与数据库迁移**：
  - 通过 `gcloud compute ssh` 登录 GCP 虚拟机（`towerpush-backend`，`34.80.200.211`），同步代码至 `76243c8`。
  - 执行 `docker compose up -d --build` 重建容器镜像，并运行 `--force-recreate` 重启 `api` / `gateway` / `battle` 容器应用新镜像。
  - 运行 DB 数据库迁移 `docker compose run --rm api /usr/local/bin/migrate`，成功应用 1 条迁移升至 schema v9 (`0009_kingdom_system`)。
  - 遵守反作弊铁律重启重放验证器 `docker restart server-verifier-1`。
  - 校验服务端健康度：`curl https://towerpushserver.jeffgame.tech/healthz` 返回 `200 ok`，Caddy CORS 及 HTTPS 响应头正常，`gateway` 配置包成功装载 `kingdom.json`。
- **Web 前端部署**：
  ...
  - 使用 `firebase deploy --project tower-push-godot` 部署至唯一的有效 Firebase Hosting 域名 (`https://tower-push-godot.web.app`；注：原 `towerpush.web.app` 已随旧项目永久作废删除)。
- **排障与排坑记录 (500 Internal Server Error)**：
  - **报错归因**：用户真人实测日志 `tower-push-godot.web.app-1784578835988.log` 显示 `GET /v5/kingdom/state` 报 500 / 404 错误。服务端 API 日志定位为 `ERROR: relation "kingdom_buildings" does not exist (SQLSTATE 42P01)`。
  - **原因**：第一次运行 `docker compose run --rm api /usr/local/bin/migrate` 时，由于旧 `api` 容器镜像尚未重新加载，运行的还是旧版 `gcp-server:dev`，导致 `0009_kingdom.up.sql` 未被执行。
  - **修复**：在容器重新拉起后二次执行 `docker compose run --rm api /usr/local/bin/migrate`，输出 `applied 1 migration(s)`，数据库成功建好 `kingdom_state` 和 `kingdom_buildings` 兩表。
  - **校验**：重新请求 `GET /v5/kingdom/state` 成功返回 `200 OK` (11ms)，王国状态拉取恢复正常。
- **结果**：
  - 服务端容器全线 Up 稳定运行，schema v9 迁移成功，全量 48 张卡牌及王国配置加载无错。

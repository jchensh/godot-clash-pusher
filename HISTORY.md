...
> **V4-S3 a~f 阶段性收尾**：a 确定性地基 → b 协议+ladder+matches → c Go gateway+battle room → d 客户端 net 层 → e 对战场景+真链路 → f 心跳+断线重连重放+超时认输。客户端单测 **217/217**；Go unit(battle 14)+integration 全过；**端到端真 WS 856 比对 0 分叉 + PG 战绩落库 + 断线重连重放 lockstep 恢复**。**lockstep 整条路线（不重写 Go 战斗逻辑、两端各跑 logic+哈希对帐）验证成立**。**只剩 g**：两台 Windows 真机实测（先重建 gateway 镜像让容器带新代码 + 改 `config/network.json` 为服务端局域网 IP）。做完 g → KAN-39 Done。

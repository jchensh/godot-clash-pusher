...
### V5 · 主界面 CR 式改版：布局/交互按新版示意图落地（🚧 代码+测试完成待真人验收，2026-07-18）

**依据**：0715 UI 系统策划的主界面示意图 [docs/design/ui_mockups/main_menu_cr_style.html](docs/design/ui_mockups/main_menu_cr_style.html)（用户已评审通过）。本步只做**布局与交互**，全部视觉为 PixelUI 占位（灰阶/像素框），正式卡通手绘资源到位后再换皮。改动 `view/main_menu.gd` 重写 + 三处返回键改道：

- **新结构**（替换旧「六按钮列表」）：①顶部 = 名片横幅(左，复用 HudWidgets.nameplate) + 货币行(右，wallet_bar 接 EconomyStateCache) + 公告(灰占位)/设置小钮；②左右活动轨 = 左挂机金库（有待领显示 +N 金币、点击直接 collect_idle 领取）/ 右探险灰占位；③中央章节主视觉占位（显示当前章「第 N 章」，整块可点进闯关地图）+ 闯关总进度条(已通关数/总关数)；④底部操作大簇 = 卡组(角标=卡组张数) | 对战大 CTA(副标=天梯·杯数) | 闯关(角标=下一关 章-关)；⑤底部五页签 = 商店(灰) 卡牌 对战(中央高亮=当前页) 闯关 探险(灰)。
...
- **验证**：客户端 **418/418** 零回归（含 test_online_runtime 服务器配置规约/test_scene_router 路由规约）；gdlint 绿。**真人验收欠着**：主界面五区布局/挂机领取/各入口跳转与返回/离线降级，见本轮对话验收用例。

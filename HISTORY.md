...
- **Web 前端部署**：
  ...
  - 使用 `firebase deploy --project tower-push-godot` 部署至 Firebase Hosting (`https://tower-push-godot.web.app` / `https://towerpush.web.app`)。
- **结果**：
  - 服务端容器全线 Up 稳定运行，schema v9 迁移完成，全量 48 张卡牌及王国配置加载无错。
  - 前端 Web 生产包发布完毕，已提供端到端真人验收测试用例单。
